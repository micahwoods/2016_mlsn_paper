...
With these parameters in Table~\ref{tab:shapetable}, one can compare Mehlich 3 soil test results for any turfgrass sample to the data used to generate the MLSN guidelines. We call this compariso the sustainability index (SI), and define it for $x$ as 1 minus the cumulative distribution function (Eq.~\ref{eq:cdf}) evaluated at $x$.  
