...
The SI for a sample with K testing at the conventional guideline of 117 mg kg\textsuperscript{-1} would be, calculating the cumulative distribution function for $P(117 \\leq x)$ of a log-logistic distribution with values for K shown in Table~\ref{tab:shapetable}, \Sexpr{formatC(siFisk(117, potassium), digits = 2, format = "f")}. The MLSN guideline levels are at an SI of 0.1. 
