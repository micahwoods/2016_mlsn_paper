...
In \cite{clarifying-3}, levels of K (116 mg kg\textsuperscript{-1}), Ca (750 mg kg\textsuperscript{-1}), and Mg (120 mg kg\textsuperscript{-1}) are given as the top of the medium range. Adding more of a particular nutrient, when the soil is above those levels, is not expected to produce a turfgrass response \citep{clarifying-3}. The CEC by summation of a soil with K at 116 116 mg kg\textsuperscript{-1}, Ca at 750 mg kg\textsuperscript{-1}, and Mg at 120 mg kg\textsuperscript{-1}, is 50 mmol\textsubscript{c} kg\textsuperscript{-1}. By selecting samples with CEC less than or equal to 50 mmol\textsubscript{c} kg\textsuperscript{-1}, we identified samples in which turf was performing well but contained nutrients in a range at which supplemental K, P, Ca, Mg, or S would typically be recommended. 

After filtering the full dataset, a total of \Sexpr{prettyNum(nPace + nAtc, big.mark = ",")} soil samples were identified with moderate pH and nutrient content in the range where fertilizer would be recommended using conventional guidelines, we began our analysis. For each of the elements of interest -- K, P, Ca, Mg, and S -- we used EasyFit software ({small{\url{www.mathwave.com}}) to evaluate a number of possible distributions for a fit to the data. Finding that a log-logistic distribution fits the K, P, Mg, and S data to a tolerable degree, and a normal distribution to the Ca data, we then used the VGAM package \citep{yee-vgam} in R \citep{r2015} to identify the scale and shape parameters. The MLSN level was selected as the value at which the probability of a sample ($X$) drawn from this distribution having a value less than or equal to the MLSN guideline is 0.1. That is, the MLSN guideline is the value of $x$ where $P(X \leq x) = 0.1$. The data and scripts to generate the guidelines are in the MLSN repository (cite GitHub).

\section*{Results}

Table~\ref{tab:sumtable} summarizes the characteristics of these data. These samples were all from good-performing turf, yet the median value for each nutrient is lower than the level for no-fertilizer required from Carrow (citation).
...
caRow <- c(length(na.omit(calcium)), mlsn.normal(calcium),
...
elements <- c("K mg/kg", "P mg/kg", "Ca mg/kg", "Mg mg/kg", "S mg/kg")

sum.table <- cbind.data.frame(elements, sum.table, carrow)

colnames(sum.table) <- c("Element", "n", "MLSN", "minimum",
                         "median", "maximum", "Conventional guideline")

sum.table <- sum.table[ , c(1, 2, 7, 4, 5, 6, 3)]

options(xtable.comment = FALSE)
options(xtable.booktabs = TRUE)
sum.table <- xtable(sum.table, caption = "Summary of MLSN data.", 
                    label = "tab:sumtable",
...
Figure~\ref{fig:histograms} shows histograms with overlying curves for the density, the normal distribution, and the fisk (log-logistic) distribution. We took the MLSN data, consisting of x samples from PACE, x from ATC, and x from GSS. These represent samples of good-performing turf in x countries and every major turfgrass species. Then we fit a model that provided a reasonable fit of these data. For K, P, Mg, and S, the fisk distribution provides a good approximation of the data. For Ca, the normal distribution was closer.
...
%----------------- need to change units in graphs to mg/kg to maintain consistency
...
\caption{ECDF for the MLSN data and for simulated data based on the Fisk distribution for K mg/kg, P mg/kg, Mg mg/kg, and S mg/kg, and based on the normal distribution for Ca mg/kg.}
\label{fig:ecdf}
\end{figure}

% text must show how to use the parameters and give some R code to generate

Table~\ref{tab:shapetable} shows the log-logistic model parameters and the $P(X \leq x) = 0.1$ level, which we identify as the MLSN guideline level. 
...
colnames(shape.table) <- c("Element", "Scale", "Shape", "$P(X \\leq x) = 0.1$ mg/kg")
...
The distribtutions can be used to calculate a sustainability index, which is the P(X) of the soil test level. With these parameters in Table~\ref{tab:shapetable}, one can put any soil sample into context. 

\section*{Discussion}

% key thing is the time the samples were collected there was enough

% point out yet to be tested but in agreement with a number of results, Woods, Petrovic, St. Johns, Snyder,
% recent Florida guy, etc
...
\begin{enumerate}
\item Figure 1. histogram MLSN
\item Figure 2. ECDF MLSN
\item Table 1. The summary of MLSN data and comparison to conventional guideline
\item Table 2. The shape and scale parameter of the fisk distribution for MLSN
\end{enumerate}
