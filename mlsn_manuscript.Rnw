...
After filtering the full dataset, we were left with \Sexpr{prettyNum(length(pace$ID) + length(atc$ID), big.mark = ",")} soil samples -- the MLSN data. These were of moderate pH and had nutrient content in the range where fertilizer would typically be recommended using conventional guidelines. 

For each of the elements of interest -- K, P, Ca, Mg, and S -- we used EasyFit software ({\small\url{www.mathwave.com}}) to evaluate a number of possible distributions for a fit to the data. Finding that a 2 parameter log-logistic (Fisk) distribution fits the K, P, Mg, and S data to a tolerable degree, and a normal distribution to the Ca data, we then used the VGAM package \citep{yee-vgam} in R \citep{r2016} to identify the scale ($b$) and shape ($k$) parameters. 

The quantile function $F^{-1}$ for this distribution is:

\begin{equation}
F^{-1}(p) = b \left(\frac{p}{1 - p}\right)^{1/k}, \quad p \in [0, 1) 
\end{equation}

The MLSN guideline was selected as the value at which the probability of a random variable ($X$) drawn from this distribution having a value less than or equal to the MLSN guideline is 0.1. That is, the MLSN guideline is the value of $x$ where $P(X \leq x) = 0.1$, obtained by evaluating the quantile function at $p = 0.1$. The scripts used to generate the MLSN guidelines are available at {\small\url{https://github.com/micahwoods/2016_mlsn_paper/tree/master/r}}.
...
Figure~\ref{fig:histograms} shows histograms with overlying curves for the density, the normal distribution, and the log-logistic distribution. For K, P, Mg, and S, the log-logistic distribution provides a good approximation of the data. For Ca, the normal distribution was a better fit. These MLSN data are representative of soil nutrient levels across a wide range of turfgrass sites, with soils that have a relatively low nutrient holding capacity, and at which the turfgrass was performing well at the time the sample was collected.
...
Figure~\ref{fig:ecdf} shows the estimated cumulative distribution function (ECDF) of the MLSN data and of simulated data generated from the fitted models. Simulated data for K, P, Mg, and S were generated using the scale and shape parameters for the log-logistic distribution from Table~\ref{tab:shapetable}. The simulated data for Ca were based on the mean and standard deviation of the Ca data. For each element, we then had a vector of soil test results from the MLSN data, and a vector of simulated data of the same length, from which we plotted the ECDF. 
...
\caption{ECDF for the MLSN data and for simulated data based on the log-logistic distribution for K, P, Mg, and S; and based on the normal distribution for Ca.}
\label{fig:ecdf}
\end{figure}

With these parameters in Table~\ref{tab:shapetable}, and the mean and standard deviation of the Ca data, one can compare Mehlich 3 soil test results for any turfgrass sample to the data used to generate the MLSN guidelines. We call this the sustainability index (SI), and it is the cumulative distribution function for any value of $X$. As an example, if one had test results showing Ca at 800 mg kg\textsuperscript{-1}, the SI would be \Sexpr{formatC(pnorm(800, mean(calcium), sd(calcium)), digits = 2, format = "f")}. 
